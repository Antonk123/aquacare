    ...
    <div className="p-5 min-h-dvh safe-area-pt">
      <div className="max-w-sm mx-auto space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-xl text-charcoal font-bold">Gå med</h1>
        </div>
        <form onSubmit={handleSubmit} className="space-y-3">
          <GlassCard>
            <div className="space-y-3">
              <div>
                <label className={LABEL_CLS}>Inbjudningskod</label>
                <input type="text" value={inviteCode} onChange={(e) => setInviteCode(e.target.value.toUpperCase())} placeholder="T.ex. A3F1B2" maxLength={6} className={`${INPUT_CLS} text-center tracking-[6px] uppercase placeholder:tracking-[2px]`} autoFocus />
              </div>
              <div>
                <label className={LABEL_CLS}>Ditt namn</label>
                <input type="text" value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="T.ex. Erik S" className={INPUT_CLS} />
              </div>
              <div>
